...
class Service(SQLModel, table=True):
    ...
    service_type: Optional[str] = Field(default=None, foreign_key="servicetype.name")
    service_resource: Optional[str] = Field(default=None)
    category: str  # Customer facing or resource facing service
...
class ServiceResource(SQLModel, table=True):
    id: str = Field(primary_key=True)
    name: str = Field(index=True, unique=True)
    description: str = Field(max_length=256, default=None)
    oper_status: int
